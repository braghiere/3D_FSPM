...
import ctypes 
from struct import *
lib1 = ctypes.cdll.LoadLibrary('../ArchiSimple/_ArchiSimple.so')
lib2 = ctypes.cdll.LoadLibrary('../ArchiSimple/_ArchiSimple2.so')
	    ...
	    min3p.gen.restart_sim= False
         ...
         while (min3p.gen.time < (min3p.gen.tfinal-tiny_time)):
	         ...
	         if (min3p.gen.varsat_flow and min3p.gen.reactive_transport):
               ...
               label .ninehundredninetyeight

               if ((min3p.chem.nutdemand_opt) and (not min3p.gen.reduce_timestep)):
                   # come back to initial time 
                   min3p.gen.time = min3p.gen.time - min3p.gen.delt
                   #same delt as no need the decrease time step for this
                   min3p.gen.delt = 1.0*min3p.gen.delt
               ...
               if (min3p.gen.time > (min3p.gen.igstime + tiny_time)):
                    min3p.gen.delt = min3p.gen.delt + min3p.gen.igstime 
                    - min3p.gen.time
               ...
               if (min3p.gen.transient_source):
                  if (min3p.gen.time > (min3p.gen.tsrc[min3p.gen.itsrc]+tiny_time)):
                     min3p.gen.delt = min3p.gen.delt + min3p.gen.tsrc[gen.itsrc] 
                     - min3p.gen.time    
		      ...
		      #write(ilog,'(/72a)')('-',i=1,72)
               ...
               if (min3p.gen.skip > 0 and min3p.gen.nskip < min3p.gen.skip):
                  #skip this time step
                  empty = 0
               elif((not min3p.chem.nutdemand_opt) or (itaum_opt_cum0)): 
               ...
               if (min3p.gen.reactive_transport):
                  ...
                  if (min3p.chem.nutdemand_opt):
                   goto .ninehundredninetyeight
                   print(' You will have to use GOTO 3 ')
                  ...
                  if(min3p.biol.coupled_archi_rld): 
                    ...
                    if (min3p.gen.time_io > (min3p.biol.time_rld - tiny_updtroot)): 
                       ...
                       if(flag == 1):
                         ...
                         lib1.COMPUTE(flag, min3p.gen.time_io, delt_MIN3P, z_max, 
                       ...
                       'j = %d, k = %d, f=point' 
                         ...
                         for ivol in range(0,min3p.gen.nn): 
                            min3p.biol.rld[ivol-1] = RSD_archi.item(ivol)
