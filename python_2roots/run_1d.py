...
import ctypes 
from struct import *
lib1 = ctypes.cdll.LoadLibrary('/home/renato/Desktop/Min3p-archi_2RSD/ArchiSimple/_ArchiSimple.so')
lib2 = ctypes.cdll.LoadLibrary('/home/renato/Desktop/Min3p-archi_2RSD/ArchiSimple/_ArchiSimple2.so')
...
def main():
    ...
    if (min3p.gen.varsat_flow):
        ...
        # !FG jan 2016 initialize second root system
        if (min3p.biol.two_plants):
           lib2.INIT_ARCHI2()
 	            ...
 	            #print(' timestep:',gen.mtime,'time:',gen.time_io,
	              ...
	              #updtrootdensity()
                    ...
                    if(min3p.biol.file_rlddata):
                      ...
                      file_rlddata = open("water.rlddata", 'w')
                    ...
                    if(min3p.biol.coupled_archi_rld): 
                      ...
                      if (min3p.gen.time_io > (min3p.biol.time_rld - tiny_updtroot)): 
                         ...
                         if(j == 0):
                            beta1 = np.ones(int(min3p.gen.tfinal))
                            beta2 = np.ones(int(min3p.gen.tfinal))

                            data1 = np.array([beta1]).T
                            data2 = np.array([beta2]).T

                            df41 = pd.DataFrame(data1)
                            df42 = pd.DataFrame(data2)

                            df41.to_excel("/home/renato/groimp_efficient/run_1d/beta_1.xls",
                                       index=False, header=False)
                            df42.to_excel("/home/renato/groimp_efficient/run_1d/beta_2.xls",
                                       index=False, header=False)

                            print ("----------------------------------------",
                                 "----------------------------------")
                            print ("--------------- Writing water.soi for", 
                                 "Min3pArchiSImple ------------------") 

                            df2 = pd.read_csv('/home/renato/groimp_efficient/run_1d/water.soi',sep='\t',names=['time','ET','canopy_int','solar_ratio','scale_tree_growth','transp_pot_sharing'])
                            ...
                            os.system("java -Xmx2000m -jar /home/renato/Downloads/GroIMP-1.5/core.jar --headless /home/renato/Downloads/FSPM_BASIC-run_1d/project.gs")

                            df1 = pd.read_csv('/home/renato/groimp_efficient/run_1d/transp.txt',header=0, names=['plant#','transpiration'],sep="\t",index_col=False)

                            PET1 = df1.loc[df1['plant#'] == 1,'transpiration'].values
                            PET2 = df1.loc[df1['plant#'] == 2,'transpiration'].values
                            ...
                            df1 = pd.read_csv('/home/renato/groimp_efficient/run_1d/root.txt',
                                          header=0, names=['plant#','rootbiom'],sep="\t",index_col=False)

                            root1 = df1.loc[df1['plant#'] == 1,'rootbiom'].values
                            root2 = df1.loc[df1['plant#'] == 2,'rootbiom'].values

                            #root = df1.rootbiom.values

                            for k in range(0,len(root1)-1):
                               root1[k] = root1[k+1] - root1[k]
                            for k in range(0,len(root2)-1):
                               root2[k] = root2[k+1] - root2[k]

                            root1 = np.array(root1)
                            root2 = np.array(root2)

                            rootbiom1 = root1*10.E-3*0.1
                            rootbiom2 = root2*10.E-3*0.1
                         ...
                         transp_pot_sharing_var = df2.transp_pot_sharing.values

                         #print (df2)
                    

                         if((PET[j] > 0.0)):
                              transp_pot_sharing_var[j] = PET1[j]/(PET1[j]+PET2[j])
                              print('transp_pot = ', transp_pot_sharing_var[j])
                         else:  
                              transp_pot_sharing_var[j] = 0.5
                              print('transp_pot = ', transp_pot_sharing_var[j])
                         #print (ET,len(ET),transp_pot_sharing_var,len(transp_pot_sharing_var))
                         ...
                         min3p.biol.transp_pot_sharing =  transp_pot_sharing_var[j]

                         print(min3p.biol.time_soi,
                               min3p.biol.pet,
                               min3p.biol.canopy_int,
                               min3p.biol.solar_ratio,
                               min3p.biol.scale_tree_growth,
                               min3p.biol.transp_pot_sharing)
                         print ("-------------------------------------",
                              "-------------------------------------")  
                         print ("------------------------ Writing update of water.soi",
                              "--------------------")
                         ...
                         data = np.array([df2.time[:i].values,
                                ...
                                df2.scale_tree_growth[:i].values,
                                df2.transp_pot_sharing[:i].values]).T
                         ...
                         df3.to_csv('/home/renato/groimp_efficient/run_1d/water.soi', sep='\t', index=False, header=None)

                         df3.to_csv('/home/renato/groimp_efficient/run_1d/water_original_%s.soi' %int(min3p.gen.time_io),sep='\t', index=False, header=None)
                      ...
                      "---------------------") 
                         ...
                         data1 = np.array([rootbiom1[:i]]).T
                         data2 = np.array([rootbiom2[:i]]).T

                         df41 = pd.DataFrame(data1)
                         df42 = pd.DataFrame(data2)

                         df41.to_csv('/home/renato/groimp_efficient/run_1d/biomrac.txt', sep='\t',index=False, header=None)
                         df42.to_csv('/home/renato/groimp_efficient/run_1d/biomrac2.txt', sep='\t',index=False, header=None)
                         df41.to_csv('/home/renato/groimp_efficient/run_1d/biomrac1_%s.txt' %int(min3p.gen.time_io), sep='\t', index=False, header=None)
                         df42.to_csv('/home/renato/groimp_efficient/run_1d/biomrac2_%s.txt' %int(min3p.gen.time_io), sep='\t', index=False, header=None)
                         ...
                         if(flag == 1):
                           ...
                           lib1.COMPUTE.argtypes = [ctypes.c_int, 
                                                 ctypes.c_float,
                                                 ctypes.c_float,
                                                 ctypes.c_int,
                                                 ctypes.c_int,
                       ...
                       'j = %d, k = %d, f=point' 
                           ...
                           if(min3p.biol.two_plants):
                            ...
                            RSD_archi2 = np.zeros((min3p.gen.nvz*min3p.gen.nvz)+1)
                            RSD_archi2 = np.asarray(RSD_archi2, dtype=np.float64)

                            lib2.COMPUTE2.argtypes = [ctypes.c_int, 
                                                 ctypes.c_float,
                                                 ctypes.c_float,
                                                 ctypes.c_int,
                                                 ctypes.c_int,
                            ...
                            for ivol in range(0,min3p.gen.nn): 
                               min3p.biol.rld2[ivol-1] = RSD_archi2.item(ivol)
        
                            min3p.biol.rld2[min3p.gen.nn-1] = 0 
                            #Au coin du domaine on met RSD = 0 sinon 
                            #on a une valeur garbage de 1
                            ...
                            ii = min3p.gen.time_io
                            if(min3p.gen.time_io < 10):
                               file_name = open("RSD2_2D_MIN3P%d.txt" % ii, 'w')
                            else:
                               file_name = open("RSD2_2D_MIN3P%d.txt" % ii, 'w')
                            if(min3p.gen.time_io >= 100):
                               file_name = open("RSD2_2D_MIN3P%d.txt" % ii, 'w')
                            ...
                            file_name.write('variables = "ivol", "x", "z", "RSD2"\n')
                            file_name.write('zone t = "RSD2, T = %f days i = %d, j = %d, k = %d, f=point\n' % (min3p.gen.time_io,min3p.gen.nvx,min3p.gen.nvz,min3p.gen.nvy))
                            for ivol in range (0,min3p.gen.nn):
                               file_name.write("%d %f %f %f\n" % (ivol,x_g.item(ivol),
                                            z_g.item(ivol),RSD_archi2.item(ivol)))
                            ...
                            ii = min3p.gen.time_io
                            if(min3p.gen.time_io < 10):
                               file_name = open("RSD_tot_2D_MIN3P%d.txt" % ii, 'w')
                            else:
                               file_name = open("RSD_tot_2D_MIN3P%d.txt" % ii, 'w')
                            if(min3p.gen.time_io >= 100):
                               file_name = open("RSD_tot_2D_MIN3P%d.txt" % ii, 'w')
                            ...
                            file_name.write('variables = "ivol", "x", "z", "RSD_tot"\n')
                            file_name.write('zone t = "RSD_tot, T = %f days i = %d, j = %d, k = %d, f=point\n' % (min3p.gen.time_io,min3p.gen.nvx,min3p.gen.nvz,min3p.gen.nvy))
                            for ivol in range (0,min3p.gen.nn):
                               file_name.write("%d %f %f %f\n" % (ivol,x_g.item(ivol),
                                z_g.item(ivol),
                                RSD_archi.item(ivol)+RSD_archi2.item(ivol)))
                           ...
                           df0 = pd.read_csv('water_%s.gsp'  %int(min3p.gen.time_io),
                               delim_whitespace=True,skiprows=3,header=None,
                               names=["x", "y", "z", "h_w", "p_w", "s_w", "theta_w",
                                      "transp", "evap","transp2", "transp_tot"])


                           transp1 = df0['transp'].values
                           transp2 = df0['transp2'].values
                           transp = df0['transp_tot'].values

                           transp1_sum = np.sum(np.array(transp1).astype(np.float))
                           transp2_sum = np.sum(np.array(transp2).astype(np.float))
                           ...
                           data1 = np.array([ET1]).T
                           data2 = np.array([ET2]).T
                           data = np.array([ET]).T


                           df81 = pd.DataFrame(data1)
                           df82 = pd.DataFrame(data2)
                           df8 = pd.DataFrame(data)

                           df81.to_csv('/home/renato/groimp_efficient/run_1d/RWU_1_%s.txt' %int(min3p.gen.time_io),
                             sep='\t', index=False, header=None )
                           df82.to_csv('/home/renato/groimp_efficient/run_1d/RWU_2_%s.txt' %int(min3p.gen.time_io),
                             sep='\t', index=False, header=None )


                           # Calculating Beta and garanteeing not greater than 1, 
                           # negative, or zero
                           if((PET1[j] >= ET1[j]) and (PET1[j] > 0.0) 
                              and (ET1[j] >= 0.0)):
                              #beta1[j] = ET1[j]/PET1[j]
                              beta1[j] = 1.0
                           else:
                              beta1[j] = 1.0
                           ...
                           # Calculating Beta and garanteeing not greater than 1, 
                           # negative, or zero
                           if((PET2[j] >= ET2[j]) and (PET2[j] > 0.0) 
                              and (ET2[j] >= 0.0)):
                              #beta2[j] = ET2[j]/PET2[j]
                              beta2[j] = 1.0
                           else:
                              beta2[j] = 1.0
                           ...
                           print('PET1=',PET1[j],'ET1=',ET1[j],'BETA_1=',beta1[j])
                           print('PET2=',PET2[j],'ET2=',ET2[j],'BETA_2=',beta2[j])

                           os.system("rm /home/renato/groimp_efficient/run_1d/beta_1.xls")
                           os.system("rm /home/renato/groimp_efficient/run_1d/beta_2.xls")
                           ...
                           df41.to_csv('/home/renato/groimp_efficient/run_1d/beta_1_%s.txt' %int(min3p.gen.time_io),
                             sep='\t', index=False, header=None)
                           df42.to_csv('/home/renato/groimp_efficient/run_1d/beta_2_%s.txt' %int(min3p.gen.time_io),
                             sep='\t', index=False, header=None)
                           ...
                           df0 = pd.read_csv('/home/renato/groimp_efficient/run_1d/field.txt',
                       delim_whitespace=True,skiprows=1,header=None,
                       names=["time", "species", "LAI", "nrShoots", "fAbs",
                              "assCO2", "biomAbove", "yield", "harvestIndex",
                              "leafArea","fieldRFR"])

                           df0.to_csv('/home/renato/groimp_efficient/run_1d/field_%s.txt' %int(min3p.gen.time_io), sep='\t', index=False, header=None)

                           df0 = pd.read_csv('/home/renato/groimp_efficient/run_1d/plant.txt',
                        delim_whitespace=True,skiprows=1,header=None,
                        names=["time", "tt", "plant", "strip", "row", "pos",
                           ...
                           df0.to_csv('/home/renato/groimp_efficient/run_1d/plant_%s.txt' %int(min3p.gen.time_io), sep='\t', index=False, header=None)
                    ...
                    sumrld = 0.0
                    for ivol in range(0,min3p.gen.nn):
                       sumrld = sumrld + min3p.biol.rld[ivol]

                    if(min3p.biol.two_plants):
                     for ivol in range(0,min3p.gen.nn):
                       sumrld = sumrld + min3p.biol.rld2[ivol]
