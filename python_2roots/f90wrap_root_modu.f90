...
subroutine f90wrap_root_modu__array__rsd_archi2(dummy_this, nd, dtype, dshape, &
    dloc)
    use gen
    use root_modu, only: root_modu_rsd_archi2 => rsd_archi2
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer, intent(out) :: nd
    integer, intent(out) :: dtype
    ...
    nd = 1
    dtype = 12
    if (allocated(root_modu_rsd_archi2)) then
        dshape(1:1) = shape(root_modu_rsd_archi2)
        dloc = loc(root_modu_rsd_archi2)
    else
        dloc = 0
    end if
end subroutine f90wrap_root_modu__array__rsd_archi2
