...
for fnumber in element:
 ...
 cs = plt.pcolormesh(Xn, Yn, data, cmap='seismic_r', vmin=-0.25, vmax=0.25)
 #cs = plt.pcolormesh(Xn2, Yn, data2, cmap='BrBG', vmin=1.e-2, vmax=5)
 cs.cmap.set_under('w')
 cbar = plt.colorbar()
 cbar.ax.set_ylabel('Anomaly Root surface density (m$^{-2}$m$^{-3}$)', rotation=270, labelpad=20)
 #xname = [-0.5,0.5]
 ...
 plt.grid()
 plt.tight_layout()
 plt.savefig('/home/renato/groimp_efficient/run_2a/figures/RSD_anom/run_2_anom_RSD_%02d.png' %(counter+1), dpi =300)
 #plt.show()
