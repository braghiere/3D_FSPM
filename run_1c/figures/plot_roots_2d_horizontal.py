...
dirname3 = "/home/renato/groimp_efficient/run_1a/"
...
for fname in filelist:
 ...
 #Inter_beta
 f22 = open(os.path.join(dirname22,fname),"r")
 ...
 f2 = interp2d(x1, z2, RSD2, kind='linear')

 f12 = interp2d(x1, z1, RSD12, kind='linear')
 f22 = interp2d(x1, z2, RSD22, kind='linear')

 f3 = interp2d(x1, z1, RSD3, kind='linear')
 f31 = interp2d(x1, z2, RSD31, kind='linear')
 ...
 data22 = f22(xnew1,ynew)

 data3 = f3(xnew1,ynew)
 data31 = f31(xnew1,ynew)

 Xn1, Yn = np.meshgrid(xnew1, ynew)
 Xn2, Yn = np.meshgrid(xnew2, ynew)

 plt.title('DAY = %d'%(counter+1))
 plt.plot(ynew,np.mean(data1,axis=0), 'k', label='Iso')
 plt.plot(ynew,np.mean(data2,axis=0), 'k--', label='Iso_beta')
 plt.plot(ynew + 0.25,np.mean(data3,axis=0), 'b--', label='Mono_beta (A)')
 plt.plot(ynew - 0.25,np.mean(data31,axis=0),'b-.', label='Mono_beta (B)')
 plt.plot(ynew + 0.25,np.mean(data12,axis=0),'r', label='Inter')
 plt.plot(ynew + 0.25,np.mean(data22,axis=0),'r--', label='Inter_beta')
 ...
 plt.xlim(0.,1.)
 plt.ylim(1.,0.0)
 plt.savefig('/home/renato/groimp_efficient/run_1c/paper_fig/RSD_horizontal/RSD_horizontal_%02d.png' %(counter+1),dpi=300)
