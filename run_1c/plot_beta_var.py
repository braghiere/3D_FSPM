...
file_name_plant = '/home/renato/groimp_efficient/run_1/plant.txt'
file_name_field = '/home/renato/groimp_efficient/run_1/field.txt'

 
file_name_plant_jules = '/home/renato/groimp_efficient/run_1c/plant_{}.txt'
file_name_field_jules = '/home/renato/groimp_efficient/run_1c/field_{}.txt'
...
print (type(transp),type(areafield))
...
df0 = pd.concat([pd.read_csv(file_name_plant_jules.format(i),sep='\t', names=["time", 
              "tt", "plant", "strip", "row", "pos", "species", "weed",
...
df2 = pd.concat([pd.read_csv(file_name_field_jules.format(i),sep='\t', names=["time", 
          "species", "LAI", "nrShoots", "fAbs", "assCO2", "biomAbove", "yield", 
          "harvestIndex","leafArea","fieldRFR"]) for i in range(59, 60)])

df3 = pd.concat([pd.read_csv(file_name_plant_clm.format(i),sep='\t', names=["time", 
              "tt", "plant", "strip", "row", "pos", "species", "weed",
              "age","nrbranches","leafArea","fpar","rfr","biom","yields","leafmass",
               "stemmass", "rootmass","shootrootratio","abovebiom","transpiration"]) for i in range(59, 60)])
...
#plt.plot(y1/1000, marker='', color=palette(1), linewidth=2.4, alpha=0.3, label='Early boot')
#plt.plot(x,y2, marker='', color=palette(2), linewidth=2.4, alpha=0.3, label='Heading')
plt.plot(y0/1000., marker='_', color=palette(4), linewidth=2.4, alpha=0.3, label='No limitation')
plt.plot(y1/1000, marker='_', color=palette(1), linewidth=2.4, alpha=0.3, label='Beta factor')
