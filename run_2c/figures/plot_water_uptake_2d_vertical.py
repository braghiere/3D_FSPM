...
for fname in filelist:
 ...
 transp12 = np.reshape(transp12, (-1, ndata1))
 transp122 = np.reshape(transp122, (-1, ndata1))
 transp1tot = np.reshape(transp1tot, (-1, ndata1))

 transp2 = np.reshape(transp2, (-1, ndata2))
 transp22 = np.reshape(transp22, (-1, ndata2))
 transp222 = np.reshape(transp222, (-1, ndata2))
 transp2tot = np.reshape(transp2tot, (-1, ndata1))
 ...
 f12 = interp2d(x1, z1, transp122, kind='cubic')
 f22 = interp2d(x2, z2, transp222, kind='cubic')
 ...
 plt.plot(np.sum(data1,axis=0), ynew, 'k', label='Iso')
 plt.plot(np.sum(data2,axis=0), ynew, 'r', label='Iso_beta')
 plt.plot(np.sum(data12,axis=0), ynew, 'k--', label='Inter')
 plt.plot(np.sum(data22,axis=0), ynew, 'r--', label='Inter_beta')

 plt.ylabel("z (m)", labelpad=20)
 plt.xlabel("Total root water uptake (m$^{-2}$m$^{-3}$)", labelpad=20)
 plt.legend()
 plt.tight_layout()
 plt.xlim(-0.05e-08,2.05e-08)
 plt.ylim(0.,1.0)
 plt.savefig('/home/renato/groimp_efficient/run_2c/paper_fig/transp_vertical/transp_vertical_%02d.png' %(counter+1),dpi=300)
 print 'transp_vertical_%02d.png' %(counter+1)
 plt.close("all")
 #plt.show()
sys.exit()
