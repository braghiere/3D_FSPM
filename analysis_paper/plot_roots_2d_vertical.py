...
for fname in filelist:
 ...
 # scipy interp. cubic
 f11 = interp2d(x1, z1, RSD11, kind='cubic')
 f12 = interp2d(x1, z2, RSD12, kind='cubic')
 ...
 #plt.semilogx(np.sum(data11,axis=1), ynew, 'k', label='Iso')
 #plt.semilogx(np.sum(data12,axis=1), ynew, 'k--', label='Iso_beta')
 #plt.semilogx(np.sum(data21,axis=1), ynew, 'b', label='Mono (plant A)')
 #plt.semilogx(np.sum(data22,axis=1), ynew, 'b--', label='Mono_beta (plant A)')
 #plt.semilogx(np.sum(data31,axis=1), ynew, 'r', label='Inter')
 #plt.semilogx(np.sum(data32,axis=1), ynew, 'r--', label='Inter_beta')

 plt.title('DAY = %d'%(counter+1))
 plt.plot(np.mean(data11,axis=1), ynew, 'k', label='Iso')
 plt.plot(np.mean(data12,axis=1), ynew, 'k--', label='Iso_beta')
 plt.plot(np.mean(data21,axis=1), ynew, 'b', label='Mono (plant B)')
 plt.plot(np.mean(data22,axis=1), ynew, 'b--', label='Mono_beta (plant B)')
 plt.plot(np.mean(data31,axis=1), ynew, 'r', label='Inter')
 plt.plot(np.mean(data32,axis=1), ynew, 'r--', label='Inter_beta')


 plt.grid(True)
 plt.ylabel("z (m)", labelpad=20)
 plt.xlabel("Mean root surface density (m$^{-2}$m$^{-3}$)", labelpad=20)
 plt.legend(loc=4)

 plt.tight_layout()
 plt.xlim(0.0,0.5)
 plt.ylim(0.,1.0)
 plt.savefig('/home/renato/groimp_efficient/analysis_paper/figures/10_cm/dicot/RSD_vertical/RSD_vertical_%02d.png' %(counter+1),dpi=300)
