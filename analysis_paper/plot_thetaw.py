...
dirname2 = "/home/renato/groimp_efficient/run_1/"
dirname1 = "/home/renato/groimp_efficient/run_1c/"
...
# cs = plt.pcolormesh(Xn2, Yn, data2, cmap='jet', vmin=min(data1.min(),data2.min()), vmax=max(data1.max(),data2.max()))
 levels = [0.14,0.15,0.16,0.17,0.18,0.19,0.2,0.21,0.22,0.23,.24,.25]
 #cs = plt.pcolormesh(Xn1, Yn, data1, cmap='coolwarm_r', vmin=0.15, vmax=0.25)

 contour = plt.contour(Xn1, Yn, data1,levels, colors='k')
 plt.clabel(contour,colors='k',linestyles='dashed')
 cs = plt.contourf(Xn1, Yn, data1,levels, cmap='coolwarm_r')
 ...
 plt.savefig('/home/renato/groimp_efficient/analysis_paper/figures/thetaw_%02d.png' %(counter + 1),dpi=300)
 #plt.show()
 print 'Figure thetaw_%02d.png saved sucessfully!' %(counter + 1)
 plt.close("all")
...
# plt.title('DAY = 94')
#plt.ylim(0.15,max(biomrsd1.max(),biomrsd2.max()))
plt.ylim(0.15,0.25)
plt.legend()
plt.title('Cereal')
plt.tight_layout()
plt.savefig('/home/renato/groimp_efficient/run_1c/paper_fig/thetaw/thetaw_total.png')
plt.show()
