...
subroutine f90wrap_biol__get__scale_tree_growth(f90wrap_scale_tree_growth)
    use biol, only: biol_scale_tree_growth => scale_tree_growth
    implicit none
    real(8), intent(out) :: f90wrap_scale_tree_growth
    
    f90wrap_scale_tree_growth = biol_scale_tree_growth
end subroutine f90wrap_biol__get__scale_tree_growth

subroutine f90wrap_biol__set__scale_tree_growth(f90wrap_scale_tree_growth)
    use biol, only: biol_scale_tree_growth => scale_tree_growth
    implicit none
    real(8), intent(in) :: f90wrap_scale_tree_growth
    
    biol_scale_tree_growth = f90wrap_scale_tree_growth
end subroutine f90wrap_biol__set__scale_tree_growth
...
subroutine f90wrap_biol__get__irlddata(f90wrap_irlddata)
    use biol, only: biol_irlddata => irlddata
    implicit none
    integer(4), intent(out) :: f90wrap_irlddata
    
    f90wrap_irlddata = biol_irlddata
end subroutine f90wrap_biol__get__irlddata

subroutine f90wrap_biol__set__irlddata(f90wrap_irlddata)
    use biol, only: biol_irlddata => irlddata
    implicit none
    integer(4), intent(in) :: f90wrap_irlddata
    
    biol_irlddata = f90wrap_irlddata
end subroutine f90wrap_biol__set__irlddata
...
subroutine f90wrap_biol__array__rldbck(dummy_this, nd, dtype, dshape, dloc)
    use parm
    use biol, only: biol_rldbck => rldbck
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer, intent(out) :: nd
    integer, intent(out) :: dtype
    ...
    nd = 1
    dtype = 12
    if (allocated(biol_rldbck)) then
        dshape(1:1) = shape(biol_rldbck)
        dloc = loc(biol_rldbck)
    else
        dloc = 0
    end if
end subroutine f90wrap_biol__array__rldbck
