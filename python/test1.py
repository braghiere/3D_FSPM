    ...
    if ((min3p.gen.varsat_flow and min3p.gen.transient_flow) or 
	 ...
	 min3p.gen.rsrt_cnt = r0
	    ...
	    min3p.gen.restart_sim= False
         ...
         j = int(min3p.gen.time)

    
         # For t = 1, initialise arrays 
         if(j == 0):
            ...
            fname = '/home/renato/groimp_efficient/beta_1.xls'
            ...
            df2 = pd.read_csv('/home/renato/groimp_efficient/input_python/feddes.soi',sep='\t',names=['time','ET','canopy_int','solar_ratio','scale_tree_growth'])
         ...
         while (min3p.gen.time < (min3p.gen.tfinal-tiny_time)):
               ...
               df3.to_csv('/home/renato/groimp_efficient/input/feddes.soi',
                          sep='\t', index=False, header=None )

               df3.to_csv('/home/renato/groimp_efficient/test/feddes_original_%s.soi' %j,sep='\t', index=False, header=None)
               ...
               df4.to_csv('/home/renato/groimp_efficient/input/biomrac.txt',
                                  sep='\t',index=False, header=None)
               df4.to_csv('/home/renato/groimp_efficient/test/biomrac_%s.txt' 
                                  %j ,sep='\t', index=False, header=None)
	         ...
	         if (min3p.gen.varsat_flow and min3p.gen.reactive_transport):
	          ...
	          #updtrootdensity()
                  ...
                  df8.to_csv('/home/renato/groimp_efficient/test/RWU_%s.txt' %j,
                              sep='\t', index=False, header=None )
                  ...
                  df4 = pd.DataFrame(data)
                  df4.to_excel("/home/renato/groimp_efficient/beta_1.xls",
                               index=False, header=False)


                  df4.to_csv('/home/renato/groimp_efficient/test/beta_1_%s.txt' 
                             %j,sep='\t', index=False, header=None)


                  df0 = pd.read_csv('/home/renato/groimp_efficient/field.txt',
                                    delim_whitespace=True,skiprows=1,header=None,
                                    names=["time", "species", "LAI", "nrShoots",
                                           "fAbs", "assCO2", "biomAbove", "yield",
                                           "harvestIndex","leafArea","fieldRFR"])

                  df0.to_csv('/home/renato/groimp_efficient/test/field_%s.txt' 
                             %j ,sep='\t', index=False, header=None)

                  df0 = pd.read_csv('/home/renato/groimp_efficient/plant.txt',
                                    delim_whitespace=True,skiprows=1,header=None,
                                    names=["time", "tt", "plant", "strip", "row",
                                           "pos", "species", "weed","age",
                                           "nrbranches","leafArea","fpar","rfr",
                                           "biom","yield","leafmass","stemmass",
                                           "rootmass","shootrootratio","abovebiom",
                                           "transpiration"])

                  df0.to_csv('/home/renato/groimp_efficient/test/plant_%s.txt' 
                             %j ,sep='\t', index=False, header=None)
