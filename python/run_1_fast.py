                    ...
                    if(min3p.biol.coupled_archi_rld): 
                      ...
                      if (min3p.gen.time_io > (min3p.biol.time_rld - tiny_updtroot)): 
                         ...
                         if(j == 0):
                            ...
                            beta = np.ones(int(min3p.gen.time_io))
                         ...
                         time_soi_var = np.arange(1.,int(min3p.gen.time_io)+1.,1.)
                         canopy_int_var = np.zeros(int(min3p.gen.time_io))
                         solar_ratio_var = np.zeros(int(min3p.gen.time_io))
                         scale_tree_growth_var = np.ones(int(min3p.gen.time_io))
                         ...
                         #data = np.array([time_soi_var[:i],
                         #       ET[:i],
                         #       canopy_int_var[:i],
                         #       solar_ratio_var[:i],
                         #       scale_tree_growth_var[:i]]).T

                         print (len(time_soi_var[:i]),
                                len(PET[:i]),
                                len(canopy_int_var[:i]), 
                                len(solar_ratio_var[:i]),
                                len(scale_tree_growth_var[:i]))
