...
for i in range(1,61):
 ...
 df1 = pd.read_csv('/home/renato/groimp_efficient/run_1/feddes_%s.gsp'%i,skiprows=3,
                   delim_whitespace=True,names=["x", "y", "z", "h_w", "p_w", "s_w", "theta_w", "transp", "evap"])

 df2 = pd.read_csv('/home/renato/groimp_efficient/run_1c/jules/feddes_%s.gsp' %i,skiprows=3,
                   delim_whitespace=True,names=["x", "y", "z", "h_w", "p_w", "s_w", "theta_w", "transp", "evap"])
 ...
 plt.pcolormesh(Xn, Yn, data1, cmap='RdBu', vmin=-0.1, vmax=0.1)
 ...
 plt.title('DAY = %d'%(i))
 plt.tight_layout()
 plt.savefig('anom_theta_%02d.png' %(i),dpi=300)
 #plt.show()
 print np.sum(RSD)
...
plt.legend()
plt.savefig('/home/renato/groimp_efficient/paper_fig/anom_theta_total.png',dpi=300)
plt.show()
